...
public:
    ...
    T* Find( char const *name ) const { for ( FileInfo *fi : list ) if ( fi && strcmp(name,fi->GetName())==0 ) return fi->GetObj(); return nullptr; }
